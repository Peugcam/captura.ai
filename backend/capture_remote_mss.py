"""
GTA Analytics - Cliente de Captura Remoto (MSS)
================================================
Captura GTA V e envia para Fly.dev (gta-analytics-v2.fly.dev)
SEM OBS! Usa MSS para contornar bloqueio do GTA.
...
async def send_frame_to_server(screenshot):
    """
    Envia frame para Fly.dev via HTTP multipart

    Args:
        screenshot: Screenshot do MSS
    ...
    try:
        # Converter MSS screenshot para PIL Image
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                print(f"[{now}] [OK] Frame {frames_sent} enviado ({size_kb} KB) | "
                      f"Uptime: {uptime}s | FPS medio: {fps_avg:.2f}")

                return True
            else:
                frames_failed += 1
                print(f"[ERROR] Erro {response.status_code}: {response.text[:100]}")
                return False

    except httpx.TimeoutException:
        frames_failed += 1
        print(f"[TIMEOUT] Servidor lento ou sem internet")
        return False
...
async def main():
    ...
    print("="*70)
    print("GTA ANALYTICS - CAPTURA REMOTA (MSS)")
    print("="*70)
    print(f"\n[*] Servidor: {SERVER_URL}")
    ...
    print(f"[*] Metodo: MSS (Multi-Screen Screenshot)")
    print("\n" + "="*70)
    ...
    try:
        ...
        monitor = sct.monitors[1]  # Monitor 1 = tela principal
        print(f"[OK] MSS inicializado com sucesso!")
        print(f"     Monitor: {monitor['width']}x{monitor['height']}")
    except Exception as e:
        print(f"[ERROR] Erro ao inicializar MSS: {e}")
        print("\n[!] Instale: pip install mss")
        return

    print("\n[*] Iniciando captura...")
    print("    (Captura tela inteira, incluindo GTA V se estiver visivel)\n")
    ...
    try:
        while True:
            try:
                ...
                # Enviar para servidor
                success = await send_frame_to_server(screenshot)
                ...
                if success:
                    ...
                    if consecutive_failures >= 5:
                        print(f"\n[WARNING] ATENCAO: {consecutive_failures} falhas consecutivas!")
                        print("   Verifique:")
                        print("   - Conexao com internet")
                        print("   - Servidor Fly.dev online: https://gta-analytics-v2.fly.dev/health")
                        print("   - Firewall nao bloqueando\n")
                        consecutive_failures = 0  # Reset para nao spammar
        ...
        print(f"\n[*] Tempo total: {uptime}s ({uptime//60}min {uptime%60}s)")
        ...
        if frames_sent > 0:
            print(f"\n[SUCCESS] {frames_sent} frames processados no servidor!")
            print(f"\n[*] Ver dashboard estrategista:")
            print(f"    https://gta-analytics-v2.fly.dev/strategist")
        else:
            print(f"\n[WARNING] Nenhum frame foi enviado.")
            print(f"    Verifique conexao com internet e servidor.")
